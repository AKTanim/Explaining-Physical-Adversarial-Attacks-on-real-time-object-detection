...
from utils_dff.activations_and_gradients import ActivationsAndGradients
from utils_dff.image import scale_cam_image, create_labels_legend, show_factorization_on_image
...
def dff(activations: np.ndarray, n_components: int = 5):
    ...
    # concepts (shape: channels x components) and explanations (shape: batch x height x width).
    ...
    def __call__(self,
        ...
        if self.computation_on_concepts:
            with torch.no_grad():
                ...
                # print(f"Concept_tensor: {concept_tensors.shape}")
                concept_outputs = self.computation_on_concepts(concept_tensors).cpu().numpy()
            # print(f"Concept_outputs_dff: {concept_outputs.shape}")
...
def run_dff_on_image(model: torch.nn.Module,
    ...
    concepts, batch_explanations, concept_outputs = dff(img_tensor[None, :], n_components)
