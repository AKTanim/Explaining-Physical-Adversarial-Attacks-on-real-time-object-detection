"""
This script defines the RTDETR class and loads the model.
Original repo: https://github.com/sovit-123/vision_transformers.git
"""
...
__all__ = ['RTDETR', ]
# This specifies that RTDETR is the only class intended for export when from module import * is used.
...
class RTDETR(nn.Module):
    ...
    def forward(self, x, targets=None):
        if self.multi_scale and self.training:
            # If multi_scale is set and the model is in training mode,
            # it resizes the input to a randomly chosen size from multi_scale
