...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def forward(self, img):
        ...
        # print(f"Raw pred {np.asarray(prediction).shape}\nRaw logits {np.asarray(logits).shape}")
        # return prediction, logits
        prediction, logits = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
                                                      agnostic=self.agnostic)

        # print(f"NMS pred {np.asarray(prediction).shape}: {prediction}\nNMS logits {np.asarray(logits).shape}")
