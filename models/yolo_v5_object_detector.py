...
class YOLOV5TorchObjectDetector(nn.Module):
    def __init__(self,
        ...
        if self.mode == 'train':
            ...
            self.model.eval()
        print(self.model)
        print(self.model.model._modules['23'].cv3)
        # fetch the names
        if names is None:
    ...
    def forward(self, img):
        ...
        print(f'predsss:  {prediction.shape} {prediction}')
        print(f'logitsss: {logits.shape} {logits}')
        # print(f"Raw pred {np.asarray(prediction).shape}: {prediction}\nRaw logits {np.asarray(logits).shape}: {logits}")
        prediction, logits = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
                                                      agnostic=self.agnostic)
        # print(f"NMS pred {len(prediction)}: {prediction}\nNMS logits {len(logits)}: {logits}")
        self.boxes, self.class_names, self.classes, self.confidences = [[[] for _ in range(img.shape[0])] for _ in
                                                                        range(4)]
        for i, det in enumerate(prediction):  # detections per image
            if len(det):
                for *xyxy, conf, cls in det:
                    xyxy = [torch.clamp(tensor, min=0) for tensor in xyxy]
                    # print(f"xyxy: {xyxy}")
