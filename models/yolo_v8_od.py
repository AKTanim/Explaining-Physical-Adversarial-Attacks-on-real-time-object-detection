...
import torch
from models.experimental_v8 import attempt_load
...
class YOLOV8TorchObjectDetector(nn.Module):
    def __init__(self,
                 model_weight,
                 device,
                 img_size,
                 ...
                 iou_thresh=0.45,
                 agnostic_nms=False):
        super(YOLOV8TorchObjectDetector, self).__init__()
        self.device = device
        self.model = None
        ...
        self.model = YOLO(model_weight) # # Download the model beforehand
    ...
    #     # print(result[1][0][0][0].max())
    #     # print(len(result[1][0][0][0]))
    #     prediction, logits = self.model(img, augment=False)
    #     # print(f"Prediction: {prediction[0][80:85]}\nLogits: {logits[0][0][0]}")
...
if __name__ == '__main__':
    ...
    model = YOLOV8TorchObjectDetector(model_path, 'cpu', img_size=(640, 640)).to('cpu')
